...
When('The user clicks Practice Questions link in Stack module', async ({ practiceQuestionPage }) => {
  await practiceQuestionPage.clickPracticeQuestions();
});

Then('The user should see Stack practice questions page', async ({ page }) => {
  await expect(page).toHaveURL(/.*stack\/practice.*/, { timeout: 10000 });
  console.log('✅ On Stack Practice Questions page');
});